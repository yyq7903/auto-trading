...
def count_lines(p):
    ...
    try:
        ...
        return 0
    if cached and cached.get("size") == stat.st_size and now - cached.get("at", 0) < 30:
...
def wallet_state():
    if WALLET_CACHE.get("data") and time.time() - WALLET_CACHE.get("at", 0) < 10:
...
def wallet_state():
    now = time.time()
    if WALLET_CACHE.get("data") and now - WALLET_CACHE.get("at", 0) < 10:
        return WALLET_CACHE["data"]
...
def svc(n):
    ...
    try:
        if os.name == "nt":
            r = subprocess.run(["wsl","bash","-lc",f"ps -ef | grep -F '{n}' | grep -v grep"],capture_output=True,text=True,encoding="utf-8",errors="ignore",timeout=2)
        ...
        if r.stdout.strip()=="active":
            ...
            return True
        p = subprocess.run(["bash","-lc",f"ps -ef | grep -F '{n}' | grep -v grep"],capture_output=True,text=True,timeout=2)
